...
// @access Public
router.get('/test', (req, res) => res.json({ msg: 'Profile works, still!' }));
...
router.get('/handle/:handle', (req, res) => {
  ...
  Profile.findOne({ handle: req.params.handle })
    ...
    .then(profile => {
      if (!profile) {
        errors.noprofile = 'No profile for the handle ' + req.params.handle;
...
router.get('/user/:user_id', (req, res) => {
  ...
  Profile.findOne({ user: req.params.user_id })
    ...
    .then(profile => {
      if (!profile) {
        errors.noprofile = 'No profile for the user: ' + req.params.user_id;
        res.status(404).json(errors);
      }
      res.json(profile);
    })
...
router.post(
  ...
  (req, res) => {
    ...
    // skills, split into an array from a csv string
    ...
    Profile.findOne({ user: req.user.id }).then(profile => {
      if (profile) {
        // update
        profileFields.updatedDate = Date.now;
